...
public record CreateRequestDto(
    [Required] Guid SiteId,
    [Required, StringLength(2000, MinimumLength = 1)] string Description,
    [Range(0.01, double.MaxValue, ErrorMessage = "Estimated cost must be positive.")] decimal EstimatedCost);

public record RejectRequestDto(
    [StringLength(2000)] string? Reason);

public record CompleteRequestDto(
    [Range(0.01, double.MaxValue, ErrorMessage = "Actual cost must be positive.")] decimal ActualCost);
